...
String connectionUrl = "jdbc:sqlserver://SQL\\ZAEPLEXLIVE;"
                 + "databaseName=Zaeplex;user=sa;password=B@ackground2017!;";  
//             String connectionUrl = "jdbc:sqlserver://TESTMACHINE\\ZAEPLEXONLINE;"
//                 + "databaseName=Zaeplex_Testing;user=sa;password=Zaeplex$2013;";  
...
public void upDateReportStatusIDForFailureInReadTimeOut(String _ssreffString,String _fName,ErrorLogCreator _err)
     {
         try
         {
             Date d=new Date();
         ...
         String query="update order_reports set ReportStatusID=98,ReportDeliverDateTime='"+strDate+"',DeliveryMethod='"+_fName+"' where orderId in (select orderID from CustomerReferenceCodeOrder where Value1='"+_ssreffString+"')";
          stmt = conn.createStatement();
         ...
         {
             if(_err!=null)
              _err.createErrorLog("upDateReportStatusID ===>"+e.getMessage());
         }
          
     }		

 public void upDateReportStatusIDForFailure(String _ssreffString,String _fName,ErrorLogCreator _err)
     {
         try
         {
             Date d=new Date();
             SimpleDateFormat fomat=new SimpleDateFormat("yyyy-MM-dd HH:mm");
            String strDate= fomat.format(d);
         String query="update order_reports set ReportStatusID=99,ReportDeliverDateTime='"+strDate+"',DeliveryMethod='"+_fName+"' where orderId in (select orderID from CustomerReferenceCodeOrder where Value1='"+_ssreffString+"')";
          stmt = conn.createStatement();
          stmt.executeUpdate(query);
         }
         catch(Exception e)
     ...
     public void upDateReportStatusID(String _ssreffString,String _fName,ErrorLogCreator _err)
     {
         try
         {
             Date d=new Date();
             SimpleDateFormat fomat=new SimpleDateFormat("yyyy-MM-dd HH:mm");
            String strDate= fomat.format(d);
         String query="update order_reports set ReportStatusID=18,ReportDeliverDateTime='"+strDate+"',DeliveryMethod='"+_fName+"' where orderId in (select orderID from CustomerReferenceCodeOrder where Value1='"+_ssreffString+"')";
          stmt = conn.createStatement();
          stmt.executeUpdate(query);
         }
         catch(Exception e)
     ...
     public HashMap<String,String>  getDataFormCriminalCaseAdditionalIdentifiersTable(String _criminalCaseID,ErrorLogCreator _errorLogObj)
     {
         
          ResultSet rs;
     HashMap<String,String>map=new HashMap<String,String>();
      if(_criminalCaseID!=null)
     if(_criminalCaseID.length()>=1)
     {
    ...
    {
        _errorLogObj.createErrorLog("getDataFormCriminalCaseAdditionalIdentifiersTable ===>"+e.getMessage());
            }
     }
    return map;
     }
public   ArrayList<HashMap<String,String>> getDataFromCriminalCaseReport(String reportsID,ErrorLogCreator _logObj)
{
   ...
   try
    ...
    {
        _logObj.createErrorLog(e.getMessage());
            }
 ...
 * @return 
 */
public String  getDisPosiTionDetails(String _detailsID,ErrorLogCreator _errErrorLogCreator )
{
    
     ResultSet rs;
     String detailsStr="";
       if(_detailsID!=null)
   if(_detailsID.length()>=1)
   {
    ...
    catch(Exception e)
    {
         ...
         _errErrorLogCreator.createErrorLog(" getDisPosiTionDetails  ===>"+e.getMessage());
            }
   }
    return detailsStr;
}

public String  getFacilitiesDetails(String _detailsID,ErrorLogCreator _errErrorLogCreator )
{
   ...
   if(_detailsID.length()>=1)
   {
    String query="select FacilityDetails  from Facilities where FacilityID="+_detailsID+"";
    try
    {
    stmt=conn.createStatement();
    rs=stmt.executeQuery(query);
    while(rs.next())
    {
        ...
        detailsStr=rs.getString("FacilityDetails");
    }
    }
    catch(Exception e)
    {
         System.out.println(" getFacilitiesDetails  ===>"+e.getMessage());
         _errErrorLogCreator.createErrorLog(" getFacilitiesDetails  ===>"+e.getMessage());
            }
   }
    return detailsStr;
}
public String getChargeLevelID(String _levelID,ErrorLogCreator _errLopg)
{
    try
    {
       Integer.parseInt(_levelID);
    }
    catch(Exception e)
    {
        return "";
     ...
     ResultSet rs;
     String detailsStr=""; String query="";
     
     if(_levelID!=null)
   if(_levelID.length()>=1)
   {
   query="select levels from ChargeLevel where levelID="+_levelID;
    try
    {
    stmt=conn.createStatement();
    rs=stmt.executeQuery(query);
    ...
    catch(Exception e)
    {
         ...
         _errLopg.createErrorLog("getChargeLevelID==>"+e.getMessage()+"query===>"+query);
            }
   }
    return detailsStr;
}
...
public String getProbationDetails(String _proBationID,ErrorLogCreator _errLopg)
{
     ResultSet rs;
     String detailsStr="";
     if(_proBationID!=null)
   if(_proBationID.length()>=1)
   {
    ...
    catch(Exception e)
    {
         System.out.println(" getProbationDetails  ===>"+e.getMessage());
            _errLopg.createErrorLog("getProbationDetails==>"+e.getMessage());
            }
   }
    return detailsStr;
}
public ArrayList<HashMap<String,String>> getDataFromcriminalChargesTable(String _crimnalId,ErrorLogCreator _erroLogObj)
{
    System.out.println("Criminal Charges ID===>"+_crimnalId);
     ResultSet rs;
    ArrayList<HashMap<String,String>>data=new ArrayList<HashMap<String,String>>();
      if(_crimnalId!=null)
    if(_crimnalId.length()>=1)
    {
...
"probationType,"
            ...
            + " where CriminalCaseID="+_crimnalId+" and isDeleted='NO'";
    try
    {
    stmt=conn.createStatement();
    rs=stmt.executeQuery(query);
...
//        map.put("FinalDisposition",""+rs.getString("FinalDisposition") );
    ...
    catch(Exception e)
    {
        ...
        _erroLogObj.createErrorLog("getDataFromcriminalChargesTable ===>"+e.getMessage());
    // String ss = in.nextLine();
            }
    }
    if(data.size()==0)
...
return status;
    ...
    public boolean hasClearRecords()
    {
        boolean status =false;
         ResultSet rs=null;
     try
 ...
 String query="select top 1  op.Orderid,op.orderreportid, crc.Value1,op.ReportResult\n" +
"from \n" +
"customer_order co inner join Order_reports op on co.orderid=op.orderid \n" +
"inner join CustomerReferenceCodeOrder crc on crc.OrderID = co.OrderID \n" +
" where op.ReportStatusID=17 and op.isDeleted='NO' and op.Active='YES' and \n" +
"co.customerID=200322 and op.ReportResult is not NULL and op.ReportResult='CLEAR'";
   stmt = conn.createStatement();
   rs = stmt.executeQuery(query);
   if(rs.next())
{
...
public ResultSet getFuilfillDataUseingRerportID(String _reportID)
{
     ResultSet rs=null;
     try
     {            

 String query="select top 25  op.Orderid,op.orderreportid, crc.Value1,op.ReportResult\n" +
"from \n" +
"customer_order co inner join Order_reports op on co.orderid=op.orderid \n" +
"inner join CustomerReferenceCodeOrder crc on crc.OrderID = co.OrderID \n" +
"where op.ReportStatusID=17 and op.isDeleted='NO' and op.Active='YES' and \n" +
"op.orderreportid="+reportID+" and op.ReportResult is not NULL ";
   stmt = conn.createStatement();
   rs = stmt.executeQuery(query);
...
public ResultSet getCriminalHistoryDataContainsHitData(int _count)
{
     ResultSet rs=null;
     try
     {            

 String query="select top "+_count+"  op.Orderid,op.orderreportid, crc.Value1,op.ReportResult\n" +
"from \n" +
"customer_order co inner join Order_reports op on co.orderid=op.orderid \n" +
"inner join CustomerReferenceCodeOrder crc on crc.OrderID = co.OrderID \n" +
" where op.ReportStatusID=17 and op.isDeleted='NO' and op.Active='YES' and \n" +
"co.customerID=200322 and op.ReportResult is not NULL and op.ReportResult='HIT' ";
   stmt = conn.createStatement();
   rs = stmt.executeQuery(query);
